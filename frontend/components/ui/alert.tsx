...
const Alert = ({
  ...
  return (
    <div className={`p-3 rounded-md flex items-center gap-x-2 text-sm ${backgroundColor} ${textColor} ${className || ""}`}>
      {icon}
      {typeof message === "string" ? (
        <p className="font-semibold" dangerouslySetInnerHTML={{ __html: message }} />
      ) : (
        message
      )}
    </div>
